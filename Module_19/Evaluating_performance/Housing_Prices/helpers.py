...
# Function only works with statsmodels.api
def append_model_stats(results, model_num):
    model_stats_dict['model'].append(f'model_{model_num}')
    model_stats_dict['fvalue'].append(results.fvalue)
    model_stats_dict['f_pvalue'].append(results.f_pvalue)
    model_stats_dict['rsquared'].append(results.rsquared)
    model_stats_dict['rsquared_adj'].append(results.rsquared_adj)
    model_stats_dict['aic'].append(results.aic)
    model_stats_dict['bic'].append(results.bic)
    df_results = pd.DataFrame(model_stats_dict)
    return df_results
...
def store_results(results):
    ...
    df_results = df_results[df_results['feature'] != 'const']

    # Create a column with absoluate values for the coefficients
    df_results['abs_coeff'] = np.abs(df_results['coeff'])
